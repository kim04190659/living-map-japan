...
export default function Page() {
  ...
  const displayedCities = cities.map(c => {
    if (scenario === "after" && scenarioData?.layerChanges[c.id]) {
      return { ...c, layer: scenarioData.layerChanges[c.id] };
    }
    return c;
  });
  ...
  if (scenario === "after" && scenarioData) {
    cities.forEach(c => {
      if (
        scenarioData.layerChanges[c.id] &&
        scenarioData.layerChanges[c.id] !== c.layer
      ) {
        diffMap[c.id] = true;
      }
    });
  ...
  return (
    <main className="min-h-screen p-6 bg-gray-50">
      ...
      <Map cities={displayedCities} diffMap={diffMap} />
    </main>
  );
}

// ================================
// Map (Client Only / Dynamic Import)
// ================================
